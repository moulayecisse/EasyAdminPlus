...
use Cisse\EasyAdminPlusBundle\Exporter\Event\EasyAdminPlusExporterEvents;
use Cisse\EasyAdminPlusBundle\Translator\Event\EasyAdminPlusTranslatorEvents;
...
trait AdminControllerTrait
{
    ...
    protected function redirectToBackendHomepage()
    {
        ...
        if (!$this->get('cisse.easy_admin_plus.acl.security.admin_authorization_checker')->isEasyAdminGranted($this->config['entities'][$homepageConfig['params']['entity']], 'list')) {
            foreach ($this->config['entities'] as $entityName => $entityInfo) {
                if ($this->get('cisse.easy_admin_plus.acl.security.admin_authorization_checker')->isEasyAdminGranted($entityInfo, 'list') &&
    ...
    protected function isActionAllowed($actionName, $checkRole = false)
    {
        if ($checkRole) {
            return false === in_array($actionName, $this->entity['disabled_actions'], true) &&
                $this->get('cisse.easy_admin_plus.acl.security.admin_authorization_checker')->isEasyAdminGranted($this->entity, $actionName);
        }
    ...
    public function exportAction()
    {
        ...
        $this->config = $this->get('cisse.easy_admin_plus.exporter.configuration.normalizer_config_pass')->process($this->config);
        $this->config = $this->get('cisse.easy_admin_plus.exporter.configuration.property_config_pass')->process($this->config);
        $this->config = $this->get('cisse.easy_admin_plus.exporter.configuration.template_config_pass')->process($this->config);
    ...
    public function loginAction()
    {
        ...
        return $this->render(
            '@CisseEasyAdminPlus/Admin/login.html.twig',
            [
                'error' => $error,
    ...
    {
        $translator = $this->get('cisse.easy_admin_plus.translator');
        ...
        if ('save' == $request->request->get('submit')) {
            ...
            // forward on GET
            $this->redirectToRoute('cisse_easy_admin_plus_translations', ['domain' => $domain]);
        }
        ...
        return $this->render('@CisseEasyAdminPlus/Admin/translations.html.twig', [
                'domains' => $domains,
                'domain' => $domain,
