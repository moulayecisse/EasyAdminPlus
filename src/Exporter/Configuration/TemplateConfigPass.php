...
namespace Cisse\EasyAdminPlusBundle\Exporter\Configuration;
        ...
        foreach ($backendConfig['entities'] as $entityName => $entityConfig) {
            foreach (array('export') as $view) {
                ...
                foreach ($entityConfig[$view]['fields'] as $fieldName => $fieldMetadata) {
                    ...
                    } elseif (file_exists('../vendor/cisse/easyadmin-plus-bundle/src/Resources/views/templates/export/field_'.$fieldMetadata['type'].'.html.twig')) {
                        $template = '@CisseEasyAdminPlus/templates/export/field_'.$fieldMetadata['type'].'.html.twig';
                    } elseif (array_key_exists($fieldTemplateName, $entityConfig['templates'])) {
                        $template = $entityConfig['templates'][$fieldTemplateName];
                    } else {
                        $template = '@CisseEasyAdminPlus/templates/export/label_null.html.twig';
                    }
